from signed_circulant_n22.model import (
    CLASS_BITS,
    CLASS_COUNT,
    EDGES,
    TARGET_MASKS,
...
def test_graph_and_class_count() -> None:
    assert len(EDGES) == 44
    assert len(set(frozenset(edge) for edge in EDGES)) == 44
    assert CLASS_BITS == 23
    assert CLASS_COUNT == 8_388_608
...
def test_twisted_classes() -> None:
    assert TARGET_MASKS == (3_495_253, 4_893_355)
