"""Explicit twelve-vertex forcing schedule."""
...
def initial_mask(n: int) -> int:
    if n < 11:
        raise ValueError("P(n,5) requires n at least eleven")
    return (1 << 10) - 1 | 1 << n | 1 << (n + 9)


def schedule(n: int) -> tuple[Force, ...]:
    if n < 11:
        raise ValueError("P(n,5) requires n at least eleven")
    result = [(index, n + index) for index in range(1, 9)]
    for index in range(n - 1, 9, -1):
        result.append(((index + 1) % n, index))
        result.append((n + ((index + 5) % n), n + index))
    return tuple(result)
...
def replay_schedule(n: int) -> UpperReplay:
    ...
    digest = hashlib.sha256(f"P({n},5)|".encode())
