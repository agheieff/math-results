...
namespace {
constexpr int k = 7;
constexpr int separator_limit = 15;
constexpr int first_order = 44;
constexpr int last_order = 56;
constexpr int power_six = 729;
constexpr std::uint64_t empty = 0;
constexpr std::uint64_t key_mask = (1ULL << 34) - 1;
  ...
  private:
    ...
    void insert_unchecked(std::uint64_t key, int cost) {
        ...
        if (slots_[index] == empty) {
            slots_[index] = key | (static_cast<std::uint64_t>(cost) << 40);
            ++size_;
        } else if (cost < static_cast<int>(slots_[index] >> 40)) {
            slots_[index] = key | (static_cast<std::uint64_t>(cost) << 40);
        }
    }
    ...
    void rehash(std::size_t capacity) {
        ...
        for (const std::uint64_t encoded : previous) {
            if (encoded != empty) {
                insert_unchecked(encoded & key_mask, static_cast<int>(encoded >> 40));
            }
        }
    }
};
...
std::uint64_t pack(int u0, int first, int last, int queue, int y_count) {
    return static_cast<std::uint64_t>(u0) |
           ...
           (static_cast<std::uint64_t>(last) << 14) |
           (static_cast<std::uint64_t>(queue) << 16) |
           (static_cast<std::uint64_t>(y_count) << 28);
}
...
void unpack(
    ...
    u0 = encoded & 3;
    first = (encoded >> 2) & 4095;
    last = (encoded >> 14) & 3;
    queue = (encoded >> 16) & 4095;
    b_count = encoded >> 40;
    y_count = (encoded >> 28) & 63;
}
...
int main() {
    ...
    for (int position = k; position < last_order; ++position) {
        ...
        for (const std::uint64_t encoded : states.slots()) {
            ...
            int y_count;
            unpack(encoded, u0, first, last, queue, b_count, y_count);
            const int oldest = queue / power_six;
            ...
            for (const auto& column : columns) {
                ...
                const int next_queue = (queue % power_six) * 3 + inner;
