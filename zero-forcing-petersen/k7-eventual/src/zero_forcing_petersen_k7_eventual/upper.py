"""Explicit sixteen-vertex forcing schedule."""
...
def initial_mask(n: int) -> int:
    if n < 15:
        raise ValueError("P(n,7) requires n at least fifteen")
    return (1 << 14) - 1 | 1 << n | 1 << (n + 13)


def schedule(n: int) -> tuple[Force, ...]:
    if n < 15:
        raise ValueError("P(n,7) requires n at least fifteen")
    result = [(index, n + index) for index in range(1, 13)]
    for index in range(n - 1, 13, -1):
        result.append(((index + 1) % n, index))
        result.append((n + ((index + 7) % n), n + index))
    return tuple(result)
...
def replay_schedule(n: int) -> UpperReplay:
    ...
    digest = hashlib.sha256(f"P({n},7)|".encode())
