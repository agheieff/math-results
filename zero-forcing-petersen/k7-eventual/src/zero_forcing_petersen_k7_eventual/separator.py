...
class SeparatorRun:
    ...
    def closure_records(self) -> list[dict[str, object]]:
        return [
            {
                "n": order,
                "boundary_at_most_fifteen_exists": count > 0,
...
def _parse_result(line: str) -> SeparatorRun:
    ...
    if not (
        len(result.layer_counts) == len(result.layer_xor) == len(result.layer_sum) == 56 - 7 + 1
    ):
...
def run_separator_dp() -> SeparatorRun:
    ...
    with tempfile.TemporaryDirectory(prefix="petersen-k7-separator-") as temporary:
