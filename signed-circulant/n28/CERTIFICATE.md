...
The path edges \(01,12,\ldots,26\,27\) are fixed positive. Bit zero is the
remaining step-1 edge \(27\,0\), and bit \(i+1\) is the step-2 edge
\(\{i,i+2\}\). Thus masks \(0\le m<2^{29}\) enumerate every switching class
exactly once.
...
The saved [summary](artifacts/n28-certificate.json) is not a trusted witness
