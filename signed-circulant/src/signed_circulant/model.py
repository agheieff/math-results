"""The source paper's spanning-tree gauge for C_20(1,2)."""
...
def switch_signing(signing: Sequence[int], switches: Sequence[int]) -> Signing:
    """Apply the vertex switching sign ``switches[u] switches[v]`` to every edge."""
...
def normalize_signing(signing: Sequence[int]) -> Signing:
    """Switch uniquely to positive signs on the path tree, fixing switch[0] = 1."""
    if len(signing) != len(EDGES):
        raise ValueError(f"expected {len(EDGES)} signs")
    switches = [1]
...
def triangle_fluxes(mask: int) -> tuple[int, ...]:
    """Signs of triangles (i,i+1,i+2), in cyclic order."""
...
def hamilton_holonomy(mask: int) -> int:
    """Sign product around the step-1 Hamilton cycle."""
...
def twisted_masks() -> tuple[int, int]:
    ...
    return tuple(sorted(masks))  # type: ignore[return-value]
