...
The path edges \(01,12,\ldots,24\,25\) are fixed positive. Bit zero is the
remaining step-1 edge \(25\,0\), and bit \(i+1\) is the step-2 edge
\(\{i,i+2\}\). Thus masks \(0\le m<2^{27}\) enumerate every switching class
exactly once.
...
The saved [summary](artifacts/n26-certificate.json) is not a trusted witness
