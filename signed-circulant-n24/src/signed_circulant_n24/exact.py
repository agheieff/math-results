...
from signed_circulant_n24.model import Matrix

Polynomial = tuple[int, ...]  # Descending coefficients.
RationalPolynomial = tuple[Fraction, ...]

TARGET_SQUARE_POLYNOMIAL: Polynomial = (1, -16, 86, -188, 142)
TARGET_POLYNOMIAL: Polynomial = (1, 0, -16, 0, 86, 0, -188, 0, 142)
EXTRA_POLYNOMIAL: Polynomial = (1, 0, -8, 0, 14)
SEPARATOR_NUMERATOR = 958
...
def expected_extremal_polynomial() -> Polynomial:
    factor = polymul(TARGET_POLYNOMIAL, EXTRA_POLYNOMIAL)
    return polymul(factor, factor)
...
def verify_target_algebra() -> None:
    """Verify radical elimination, target identification, and the separator."""
    # For z=alpha-4, eliminating sqrt(2) and sqrt(3) gives
    # (z^2+1)^2-3(2z+1)^2 = p(alpha).
    z_squared_plus_one: Polynomial = (1, -8, 17)
    two_z_plus_one: Polynomial = (2, -7)
    eliminated = polyadd(
        polymul(z_squared_plus_one, z_squared_plus_one),
        tuple(-3 * coefficient for coefficient in polymul(two_z_plus_one, two_z_plus_one)),
    )
    if eliminated != TARGET_SQUARE_POLYNOMIAL:
        raise AssertionError("target radical elimination identity failed")

    # sqrt(3)>5/3, sqrt(6)>12/5, and sqrt(2)>7/5 give alpha>227/30>7.
    if not (
        Fraction(5, 3) ** 2 < 3
        and Fraction(12, 5) ** 2 < 6
        and Fraction(7, 5) ** 2 < 2
        and 4 + Fraction(5, 3) + (Fraction(12, 5) + Fraction(7, 5)) / 2 == Fraction(227, 30)
        and Fraction(227, 30) > 7
    ):
        raise AssertionError("rational target lower bound failed")

    intervals = (
        (Fraction(17, 10), Fraction(9, 5)),
        (Fraction(27, 10), Fraction(14, 5)),
        (Fraction(19, 5), Fraction(39, 10)),
        (Fraction(7663, 1000), Fraction(SEPARATOR_NUMERATOR, SEPARATOR_DENOMINATOR)),
    )
